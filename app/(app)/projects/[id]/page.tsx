...
type Tab = "overview" | "estimate" | "budget" | "tasks" | "notes" | "files" | "activity";
type SheetTab = "sheet" | "estimator" | "info";

const TABS: { id: Tab; label: string; cap?: string }[] = [
  { id: "overview", label: "Overview" },
  { id: "estimate", label: "Estimate", cap: "estimates.view" },
  { id: "budget", label: "Budget", cap: "budgets.view" },
  { id: "tasks", label: "Tasks & checklist" },
  { id: "notes", label: "Notes" },
  { id: "files", label: "Plans · files · photos", cap: "files.view" },
  { id: "activity", label: "Activity", cap: "audit.view_project" },
];
...
function ProjectWorkspace() {
        ...
        <main className="mx-auto max-w-lg px-4 py-24 text-center">
          <div className="panel bg-paper p-10">
            ...
            <p className="mt-3 text-sm text-mute">
              {storageError ?? "It may have been deleted, or it hasn't been shared with you."}
            </p>
  ...
  const tabBtn = (t: Tab, label: string) => (
    <button
      key={t}
      onClick={() => setTab(t)}
      className={`border px-3 py-2 font-mono text-[0.6875rem] uppercase tracking-[0.14em] transition-colors sm:px-4 ${
        tab === t
  ...
  const subTab = (t: SheetTab, label: string) => (
    <button
      ...
      className={`px-3 py-1 font-mono text-[0.625rem] uppercase tracking-[0.14em] transition-colors ${
        sheetTab === t ? "bg-ink text-paper" : "text-mute hover:bg-ink/10 hover:text-ink"
      }`}
    >
      {label}
    </button>
    ...
    <div className="sheet-grid min-h-screen">
      <div className="no-print">
        ...
        <div className="border-b bg-paper">
          <div className="mx-auto max-w-7xl px-4 py-4">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
              ...
              {canSeeMoney && totals && (
                <span className="tnum ml-auto font-mono text-sm">{money(totals.grand)}</span>
              )}
            </div>
            <div className="mt-1 flex flex-wrap items-end justify-between gap-3">
              {canEditEstimate ? (
                ...
                <h1 className="font-display min-w-0 flex-1 text-xl md:text-2xl">{project.name}</h1>
              )}
              {tab === "estimate" && totals && (
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">{visibleTabs.map((t) => tabBtn(t.id, t.label))}</div>
          </div>
        </div>
        ...
        <main className="mx-auto max-w-7xl px-4 py-5 pb-24">
          {tab === "overview" && (
            <OverviewPanel projectId={id} companyId={companyId} project={project} totals={canSeeMoney ? totals : null} onOpenTab={(t) => setTab(t as Tab)} />
          )}
          ...
          {tab === "estimate" && (
            <>
              <div className="mb-4 flex items-center gap-1 border-b pb-3">
                ...
                {!canEditEstimate && (
                  <span className="microlabel ml-auto">Read only · your role can view but not edit</span>
                )}
              </div>
              {!estimateId && (
                <p className="mb-4 border border-dashed px-3 py-2 text-xs text-mute">
                  This project has no estimate sheet yet.
                </p>
              )}
          ...
          {tab === "budget" && (
            <BudgetPanel projectId={id} companyId={companyId} project={project} canEdit={session.can("budgets.edit")} />
          )}
          {tab === "tasks" && (
            <TasksPanel
              projectId={id}
              ...
              canManage={session.can("tasks.manage")}
              canComplete={session.can("tasks.complete")}
            />
          )}
          {tab === "notes" && (
            <NotesPanel
              ...
              canCreate={session.can("notes.create")}
              canManage={session.can("notes.manage")}
            />
          )}
          {tab === "files" && (
            <FilesPanel
              ...
              companyId={companyId}
              userId={session.userId}
              canUpload={session.can("files.upload")}
              canDeleteAny={session.can("files.delete")}
            />
          )}
      ...
      {canEditEstimate && estimateId && <BobChat project={project} update={update} />}
